...
model = ChatOpenAI(model="gpt-4o-mini", api_key = openai_api)
class objstate(TypedDict):
...
state: objstate = {'active_agent': active_agent}
...
def search_flights(
    ...
    state['active_agent'] = 'flight_assistant'

    with open("state.txt","w") as f:
        f.write(state['active_agent'])
...
def search_hotels(location: str) -> list[dict]:
    ...
    """
    state['active_agent'] = 'hotel_assistant'
...
threads = {'configurable':{'thread_id':'1'}}
...
data = app.invoke(query,config=threads)

for obj in data['messages']:
    print(obj.content)

print(state['active_agent'],end = '-'*50)
